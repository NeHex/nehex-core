...
@router.get("/dailies/{daily_id}", response_model=AdminDailyDetailResponse, summary="Get daily detail")
def admin_get_daily(
    daily_id: int,
    _: AdminPrincipal = Depends(require_admin_principal),
    session: Session = Depends(get_db_session),
) -> AdminDailyDetailResponse:
    item = get_daily_by_id(session=session, daily_id=daily_id)
    if item is None:
        raise HTTPException(status_code=404, detail="Daily not found")
    return AdminDailyDetailResponse(data=item)
...
@router.get("/pages/{page_id}", response_model=AdminPageDetailResponse, summary="Get standalone page detail")
def admin_get_page(
    page_id: int,
    _: AdminPrincipal = Depends(require_admin_principal),
    session: Session = Depends(get_db_session),
) -> AdminPageDetailResponse:
    item = get_page_by_id(session=session, page_id=page_id)
    if item is None:
