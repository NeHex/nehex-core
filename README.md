...
- 版本：后端 `APP_VERSION` 升级为 `v1.0.1`，`/version` 接口返回同步更新。
- 版本：后台前端版本升级为 `v1.0.1`（`app/nehex-admin/package.json` 与 lockfile 顶层版本号同步）。
- 功能：新增设定下的通知设置与邮件管理能力（SMTP 测试、评论通知模板、发送记录分标签查看）。
- 功能：新增设定下的备份与恢复能力，支持备份列表下载、覆盖恢复、上传备份文件恢复。
- 稳定性：修复 MySQL 恢复时 `slow_log` 等系统日志表导致的恢复失败问题。
- 安全性：备份恢复流程增强，上传改为流式写入并增加压缩包安全解包校验。
